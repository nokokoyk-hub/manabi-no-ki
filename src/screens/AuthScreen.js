...
// 🔐 AuthScreen - ログイン画面
// まなびの木 - Phase A 認証基盤
// v1.0.0: 新規作成（2026/06/18）
// ============================================
// Googleログイン（メイン）+ マジックリンク（フォールバック）
// WebView検知 + ヘルプ案内
// ============================================
...
// ------------------------------------------
// WebView判定
// LINE / Facebook / Instagram / Yahoo 等の
// アプリ内ブラウザを検知
// ------------------------------------------
const isWebView = () => {
...
function AuthScreen() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // 成功時はGoogleにリダイレクトされるので、isLoadingはそのまま
    } catch (err) {
  ...
  const handleMagicLink = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setMessage('OK_✉️ メールを おくったよ！\nメールの なかの リンクを タップしてね');
      }
    } catch (err) {
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <div style={styles.emailSection}>
          ...
          >
            {isLoading ? 'おくっています...' : 'ログインリンクを おくる'}
          </button>
        </div>
