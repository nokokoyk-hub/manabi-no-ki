...
|------|-----|
| 現在のバージョン | **v0.5.0** |
| APP_VERSION | `src/App.js` → `APP_VERSION = '0.5.0'` ✅ |
| package.json | `"version": "0.5.0"` ✅ |
| version.json | `public/version.json` → `"version": "0.5.0"` ✅ |
...
> v0.5.0時点の教科別レベル設定は、DBスキーマ変更を避けるため端末localStorage保存で導入。復習画面は既存の `learning_sessions` を読み取り、モード単位でおすすめを出す。問題ごとの誤答記録は未実装。
...
├── src/
│   ├── App.js               # メインルーター + Supabase連携 + レベル設定 + 復習 + 更新通知
│   ├── index.js
│   ├── index.css
│   ├── lib/
│   │   ├── supabase.js
│   │   └── storage.js        # データアクセス + localStorage設定 + 最近のセッション取得
│   ├── constants/
│   │   ├── colors.js
...
│   │   └── UpdateBanner.js
│   ├── screens/
│   │   ├── HomeScreen.js          # ホーム画面（まめ + レベル設定 + 復習導線）
│   │   ├── LearningScreen.js      # 学習画面（教科別レベル・復習モード対応）
...
| 使用場所 | ホーム画面、学習画面、ローディング、レベル設定画面、復習画面 |
...
| ふくしゅう画面 | 最近14日の学習記録から正解率の低いモードをおすすめ表示 | FukushuScreen.js, storage.js |
| ふくしゅう開始 | さんすう/こくご/おくりがな/とけい等の復習モードで学習開始 | FukushuScreen.js, LearningScreen.js |
...
既存の `questions.js` は温存し、v0.5.0では `levelQuestions.js` でgradeLevel付与・追加問題・出題フィルターを担当する。
...
- 過去14日の `learning_sessions` を読み取り
- モードごとの正解率を計算
- 正解率が低いモードを上に表示
- データが少ない場合は、さんすう/おくりがな/とけい/こくごを入口として表示
- ふくしゅう画面から学習画面へ遷移

### 現在の限界
- 問題ごとの正誤は保存していないため、「どの問題を間違えたか」までは追えない
- 現状はモード単位の弱点推定
- より精密な復習には `learning_answers` のような回答履歴テーブルが必要
...
1. **v0.5.0動作確認**
   - ホームの「ふくしゅう」カードが押せるか
   - 復習画面にまめとおすすめカードが表示されるか
   - 学習履歴がある場合、正解率が低いモードが上に出るか
   - 学習履歴がない場合、初期おすすめが表示されるか
   - さんすう/こくご/おくりがな/とけいの復習開始が正しいモードで開くか
   - 完了後にホームへ戻り、セッション記録が保存されるか
2. **Vercelデプロイ確認**
3. **問題ごとの誤答記録を入れるか判断**
   - 必要ならSupabaseに回答履歴テーブル追加
4. **ごほうび画面の実装**
...
8. **anon keyの安全性**: Supabase anon keyは公開前提。RLS設定済み。service_role keyは絶対にコードに書かないこと
...
| 5/30 | ホームの「ふくしゅう」が準備中なので何とかしたい | ✅ v0.5.0で初回実装 |
